...
* jdbc 보다는 mvc 패턴이 뭔지 이해하는 데 어려움이 있었습니다.
* 기존에 하나의 코드에서 처리하던 형식에서 코드를 controller, dao로 분리하려니 어떻게 코드를 구분할지 헷갈렸는데
* 미니 프로젝트라고 칭하는 실습 2번을 통해 각 패키지의 역할을 명확히 이해할 수 있었습니다.
  * controller 단은 클라이언트의 응답을 받아들이는 역할을 하고, dao는 데이터베이스와 연동하는 역할을 한다는 것을 알았습니다.
  * 이외에도 service 단에서 비즈니스 로직을 처리한다는 것을 이해했습니다.
* PreaparedStatement 및 CallableStatement의 경우 이미 기존에 Mysql을 통해 SQL문에 대한 학습을 진행했기 때문에 원리를 이해하고 실습하는 데 어려움이 있지 않았습니다.
* 이를 통해 제 안에서 프로그래밍이 좀 더 객체지향적인 방식으로 확장되었다는 생각이 듭니다.
